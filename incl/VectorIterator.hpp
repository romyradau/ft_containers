...
namespace ft{
	...
	class vec_iterator{
		...
		public:
			TODO://do i need the empty constructor??
			vec_iterator(void): it_ptr() {}
			...
			//post operator
			//returns a copy of the it, bc the object in place should not be modified
			vec_iterator& operator--() {--it_ptr;return *this;}
  			vec_iterator operator--(int) {vec_iterator tmp(*this); operator--(); return tmp;}
			...
			//goes to the bool==operator of teh pointer object
  			bool operator!=(vec_iterator const & rhs) const {return it_ptr!=rhs.it_ptr;}
			// vec_iterator& operator+(int n) {return }
